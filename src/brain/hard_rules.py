...
def check_hard_rules(
    ...
    if avg_cost and current_price and avg_cost > 0:
        ...
        if drawdown_pct > config.RULE_AVERAGING_DOWN_DRAWDOWN * 100 and num_buys >= config.RULE_AVERAGING_DOWN_MAX_BUYS:
            return (
    ...
    if total_value and (position_value / total_value) > config.RULE_MAX_POSITION_PCT:
        return f"POSITION LIMIT: already >{config.RULE_MAX_POSITION_PCT:.0%} of portfolio (${position_value:,.0f})"

    theme = guidance.get("theme")
    if theme and theme != config.RULE_THEME_CAP_EXEMPT:
        theme_pct = portfolio_summary.get("theme_allocations", {}).get(theme, 0)
        if theme_pct > config.RULE_MAX_THEME_PCT:
            return f"THEME LIMIT: {theme} theme already at {theme_pct:.0%} of portfolio (max {config.RULE_MAX_THEME_PCT:.0%})"
